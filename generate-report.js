...
const authNM    = readJSON('contract/auth-results.json');
const profNM    = readJSON('contract/profile-results.json');
...
const authContract = parseNewman(authNM,  'Auth API');
const profContract = parseNewman(profNM, 'Profile API');
...
function parseK6(data) {
  ...
  // Values are directly on the metric object (k6 --summary-export format)
  const get = (key, stat) => {
    const entry = m[key];
  ...
  // Thresholds are nested per-metric: { "rate<0.01": false } where false = passed
  const thresholds = [];
  let anyFailed = false;
...
function zapRiskLabel(a) {
  ...
  // fallback: extract first word from riskdesc (before space/parenthesis)
...
const contractTotal  = authContract.total + profContract.total;
const contractFailed = authContract.failed + profContract.failed;
...
function newmanSection(auth, prof) {
  const status = stageStatus('Contract Tests (Newman)');
  const dur    = stageDuration('Contract Tests (Newman)');
  const total  = auth.total + prof.total;
  const passed = auth.passed + prof.passed;
  const failed = auth.failed + prof.failed;
  ...
  <section class="stage-section">
    ...
    <div class="stage-metrics">
      ...
      ${metricCard('Collections', '2')}
    </div>
    ${collectionTable(auth)}
    ${collectionTable(prof)}
  </section>`;
}
...
<title>Pipeline Report — ${runAt}</title>
<style>
/* ── Reset & base ── */
...
<div class="page">
  ...
  <header class="hero">
    <div class="hero-top">
      <h1 class="hero-title">CI/CD Pipeline Report</h1>
  ...
  ${newmanSection(authContract, profContract)}
